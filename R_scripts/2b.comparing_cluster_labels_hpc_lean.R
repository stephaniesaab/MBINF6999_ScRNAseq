...
cat("the manual clusters are ", length(unique(singleR_rds$seurat_clusters)), "\n") #NULL --> No manual clusters
...
srt_rds@meta.data$singleR_labels <- singleR_labels

#Save the RDS with singleR labels
saveRDS(srt_rds, "~/links/scratch/data/merged_labels_seurat_object_lean.rds")
