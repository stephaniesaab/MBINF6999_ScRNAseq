#3b.graphing_new_labels_hpc_lean.R
...
out_dir <- "~/links/scratch/plots/lean_v2"
#Load libraries ----------------------------------------------------------------
library(SingleR)
...
#Save SingleR plot
save_plot("~/links/scratch/plots/dim_plot_singleR_lean.png", dim_plot_singleR_clusters, base_height = 7, base_width = 8)
...
png(paste0(out_dir, "hist_scores_below_0.18_lean.png"), width = 1800, height = 1500, res = 300)
hist(low_scores,
     col = "skyblue",
     main = "Histogram of low Delta scores (below median) V2",
     xlab = "Delta scores")
dev.off()
...
vio_box_plot_cluster_scores <- ggplot(cluster_scores, aes(x = labels, y = median_score, fill = labels)) +
  ...
  theme_minimal() +
  labs(
    title = "Distribution of SingleR Delta Scores across Labels V2",
...
ggsave(paste0(out_dir, "violin_cluster_scores_lean.png"), plot = vio_box_plot_cluster_scores, width = 10, height = 7, dpi = 300)
...
ggsave(paste0(out_dir, "ridgeplot_cluster_scores_lean.png"), plot = ridgeplot_cluster_scores, width = 8, height = 11, dpi = 300)
...
contingency_matrix <- ggplot(df_contingency_table, aes(x = Manual, y = SingleR, fill = Proportion, text = hover_text)) +
  ...
  labs(title = "SingleR vs. Manual Cluster Comparison V2", x = "Manual Labels", y = "SingleR Labels")
...
htmlwidgets::saveWidget(interactive_contingency_table, file=paste0(out_dir, "interactive_contingency_table_lean.html"), selfcontained = TRUE)
